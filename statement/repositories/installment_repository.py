...
def add_installments(transactions, new_transaction):
    ...
    for i in range(start_index, final_index):
        new_transaction.payment_date = transaction_repository.add_month(
            new_transaction, i
        )
...
def remove_installments(transactions, new_transaction):
    ...
    if new_transaction.installments_number <= 1:
        ...
        transaction_services.update_transaction(
            transactions[0], new_transaction
        )
        installment_services.delete_installment(installment)
    else:
        for index, transaction in enumerate(transactions):
            if transaction.paid <= new_transaction.installments_number:
                new_transaction.payment_date = (
                    transaction_repository.add_month(transaction, index)
                )
                new_transaction.paid = index + 1
                transaction_services.update_transaction(
        ...
        installment_services.update_installment(
            transactions[0].installment, new_transaction.installment
        )
...
def update_installment(transactions, new_transaction, *args):
    increase_installments_number = (
        transactions[0].installments_number
        < new_transaction.installments_number
    )
    decrease_installments_number = (
        transactions[0].installments_number
        > new_transaction.installments_number
    )
...
def validate_installment(old_transaction, new_transaction):
    add_installment = (
        old_transaction.installments_number
        < new_transaction.installments_number
    )
    remove_installment = (
        old_transaction.installments_number
    ...
    if add_installment:
        if not old_transaction.installment:
            installment = Installment(
                date.today(), new_transaction.description, new_transaction.user
            )
            installment_db = installment_services.create_installment(
                installment
            )
...
def advance_installments(quantity, initial_date, installments):
    ...
    if initial_date > installments[0].release_date:
        next_expiration = find_next_expiration_card_date(
            installments[0].card, initial_date
        )
    else:
        next_expiration = find_next_expiration_card_date(
            installments[0].card, installments[0].release_date
        )
    for index, installment in enumerate(installments):
        if installment.payment_date > initial_date:
            if advanced < quantity:
                ...
                not_advanced += 1
                installment.payment_date = next_expiration + relativedelta(
                    months=not_advanced
                )
                transaction_services.update_transaction(
                    installment, installment
...
def find_next_expiration_card_date(card, new_date):
    next_expiration_date = date(
        new_date.year, new_date.month, card.expiration_day
    )
